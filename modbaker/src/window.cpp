...
c_window_manager::c_window_manager()
{
	...
	try
	{
		...
		// Init the graphics
		graphics = new gcn::OpenGLGraphics(g_renderer.get_screen()->w, g_renderer.get_screen()->h);
		graphics->setTargetPlane(g_renderer.get_screen()->w, g_renderer.get_screen()->h);
	...
	//	font = new gcn::contrib::OGLFTFont(pfname.c_str(), g_config.get_font_size());
		...
		top->setDimension(gcn::Rectangle(0, 0, g_renderer.get_screen()->w, g_renderer.get_screen()->h));
		top->setOpaque(false);
...
//		string file;
//		file = g_config.get_egoboo_path() + "modules/" + g_mesh->modname + "/gamedat/tile0.bmp";
//		cout << "file: " << file << endl;
//		this->add_icon(this->c_texture, file,   0,  0, ACTION_MESH_NEW);
