...
void draw_one_character_icon( const CHR_REF & item, float x, float y, bool_t draw_ammo )
{
    ...
    ego_chr * pitem = !INGAME_CHR( item ) ? NULL : ChrObjList.get_pdata( item );
...
void update_all_chr_instance()
{
    ...
    {
        ego_chr * pchr = ChrObjList.get_pdata( cnt );
        if ( NULL == pchr ) continue;
...
bool_t ego_billboard_data::update( ego_billboard_data * pbb )
{
    ...
    pchr = ChrObjList.get_pdata( pbb->ichr );
...
void BillboardList_update_all()
{
    ...
    for ( cnt = 0; cnt < BILLBOARD_COUNT; cnt++ )
    {
        ...
        if ( !INGAME_CHR( pbb->ichr ) || IS_ATTACHED_PCHR( ChrObjList.get_pdata( pbb->ichr ) ) )
        {
            is_invalid = btrue;
        }
...
bool_t render_billboard( ego_camera * pcam, ego_billboard_data * pbb, float scale )
{
    ...
    if ( INGAME_CHR( pbb->ichr ) && IS_ATTACHED_PCHR( ChrObjList.get_pdata( pbb->ichr ) ) ) return bfalse;
