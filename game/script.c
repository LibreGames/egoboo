...
void load_ai_codes( char* loadname )
{
  ...
  REGISTER_OPCODE( opcode_lst, 'V', VAR_SPAWN_DISTANCE,  "selfspawndistance" );   // 071
  ...
  REGISTER_OPCODE( opcode_lst, 'V', VAR_TARGET_RELOADTIME,"targetreloadtime" );   // 074
  ...
  REGISTER_FUNCTION_ALIAS( opcode_lst, SetTargetToWhoeverAttacked, "SetTargetToAttacker" );
  REGISTER_FUNCTION_ALIAS( opcode_lst, SetTargetToWhoeverIsHolding, "SetTargetToHolder" );
...
bool_t run_function( Game_t * gs, Uint32 value, CHR_REF ichr )
{
  ...
  switch ( opcode )
  {
    ...
    case F_SetTime:
      // This function resets the timer to tmpargument (if its not negative)
	  if(pstate->tmpargument > -1) pstate->time = pstate->tmpargument;
      break;
    ...
    case F_IfTargetHasSkillID:
      // This function proceeds if ID matches tmpargument
      returncode = (0 != check_skills( gs, pstate->target, pstate->tmpargument ) );
      break;
    ...
    case F_SetSpeedPercent:
	  //This sets an exact percent limit (or bonus) to the characters speed
      chr_reset_accel( gs, ichr );
    ...
    case F_IfStateIsOdd:
	  //Proceeds if the AI state is not even
    ...
    case F_IfHolderScoredAHit:
	  ...
	  // Sets target to the one who was hit by the attack
      returncode = bfalse;
      if ( chr_attached( chrlst, chrlst_size, ichr ) )
      {
        pstate->tmpargument = HAS_SOME_BITS( chrlst[pstate->target].aistate.alert, ALERT_SCOREDAHIT );
        pstate->target = chr_get_aihitlast( chrlst, chrlst_size, chrlst + chr_get_attachedto( chrlst, chrlst_size, ichr ) );
		returncode = btrue;
      }
      break;
    ...
    case F_IfHolderBlocked:
	{
	  ...
	  //Sets the target to the one who attack became blocked
	  CHR_REF attacklast = chr_get_aiattacklast( chrlst, chrlst_size, chrlst + ichr );
      returncode = bfalse;
      if ( chr_attached( chrlst, chrlst_size, ichr ) )
      {
        pstate->tmpargument = HAS_SOME_BITS( chrlst[pstate->target].aistate.alert, ALERT_BLOCKED );
        pstate->target = attacklast;
		returncode = btrue;
      }
	}
    ...
    case F_GetTargetSkillLevel:
      //Sets tmpargument to the level of the skill specified in tmpdistance
