...
/*
 * Name:
 *     sdlglcfgWriteComment
 * Description:
 *     Writes the first comment line for editor files
 * Input:
 *     f *: Pointer on FILE  
 *     fname *:    Name of file to write
 *     lineinfo *: Pointer on description how to write the data
 */
 static void sdlglcfgWriteComment(FILE *f)
 {
    fputs("//File generated by EGOBOO - Map Editor V 0.2.0\n", f);
 }
...
static void sdlglcfgValToStr(char *val_str, char valtype, void *value, char vallen)
{
    ...
    switch(valtype)
    {
        ...
        case SDLGLCFG_VAL_FLOAT:
            sprintf(val_str, "%.2f", *(float *)value);
            break;
    ...
    if (f)
    {
        while (sdlglcfgGetValidLine(f, line, SDLGLCFG_LINELEN - 2, '/'))
        {
            ...
            {   
                ...
                // Support '#dependency' ==> e.g "Advent 0" object-name / slot-no
                pbrk = strchr(line, '#');
                if(pbrk)
                {
