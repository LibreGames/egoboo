...
// ---------------------------------------------------------------------------------------------------------------------------------
    ...
    #define    m_assert(cond) assertcond)
#endif
...
static    const    char        *sourceFile[256]            = {"??", NULL};
static    const    char        *sourceFunc[256]            = {"??", NULL};
static        unsigned int     sourceLine[256]            = {   0, 0   };
static  int                  sourceDepth = 0;
...
static    const    char        *get_sourceFile()
{
    return sourceDepth < 256 ? sourceFile[sourceDepth] : "??";
}

static    const    char        *get_sourceFunc()
{
    return sourceDepth < 256 ? sourceFunc[sourceDepth] : "??";
}

static        unsigned int     get_sourceLine()
{
    return sourceDepth < 256 ? sourceLine[sourceDepth] : 0;
}
...
static    const char    *ownerString(const char *sourceFile, const unsigned int _sourceLine, const char *_sourceFunc)
{
    static    char    str[90];
    memset(str, 0, sizeof(str));
    sprintf(str, "%s(%05d)::%s", sourceFileStripper(sourceFile), _sourceLine, _sourceFunc);
    return str;
}
...
void    m_setOwner(const char *file, const unsigned int line, const char *func)
{
    ...
    {
        log("[I] NOTE! Possible destructor chain: previous owner is %s", ownerString(get_sourceFile(), get_sourceLine(), get_sourceFunc()));
    }
    ...
    if( sourceDepth < 256 )
    {
        sourceFile[sourceDepth] = file;
        sourceLine[sourceDepth] = line;
        sourceFunc[sourceDepth] = func;
    }
...
static    void    resetGlobals()
{
    ...
    {
        sourceDepth = 0;
        sourceFile[sourceDepth] = "??";
        sourceLine[sourceDepth] = 0;
        sourceFunc[sourceDepth] = "??";
    }
}
...
void    *operator new(size_t reportedSize)
{
    ...
    const    char           *file = get_sourceFile();
    const    unsigned int    line = get_sourceLine();
    const    char           *func = get_sourceFunc();
...
void    *operator new(size_t reportedSize, const char *sourceFile, int _sourceLine)
{
    #ifdef TEST_MEMORY_MANAGER
    log("[D] ENTER: new");
    #endif
    ...
    for(;;)
    {
        ...
        void    *ptr = m_allocator(sourceFile, _sourceLine, "??", m_alloc_new, reportedSize);
        if (ptr)
        {
            #ifdef TEST_MEMORY_MANAGER
            log("[D] EXIT : new");
...
void    *operator new[](size_t reportedSize, const char *sourceFile, int _sourceLine)
{
    ...
    for(;;)
    {
        ...
        void    *ptr = m_allocator(sourceFile, _sourceLine, "??", m_alloc_new_array, reportedSize);
        if (ptr)
        {
            #ifdef TEST_MEMORY_MANAGER
            log("[D] EXIT : new[]");
...
void    operator delete(void *reportedAddress)
{
    ...
    if (reportedAddress) m_freeator(get_sourceFile(), get_sourceLine(), get_sourceFunc(), m_alloc_delete, reportedAddress);
    else if (alwaysLogAll) log("[-] ----- %8s of NULL                      by %s", allocationTypes[m_alloc_delete], ownerString(get_sourceFile(), get_sourceLine(), get_sourceFunc()));
...
void    operator delete[](void *reportedAddress)
{
    ...
    if (reportedAddress) m_freeator(get_sourceFile(), get_sourceLine(), get_sourceFunc(), m_alloc_delete_array, reportedAddress);
    else if (alwaysLogAll)
        log("[-] ----- %8s of NULL                      by %s", allocationTypes[m_alloc_delete_array], ownerString(get_sourceFile(), get_sourceLine(), get_sourceFunc()));
...
void    *m_allocator(const char *sourceFile, const unsigned int _sourceLine, const char *_sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
    try
    {
        #ifdef TEST_MEMORY_MANAGER
        ...
        if (alwaysLogAll) log("[+] %05d %8s of size 0x%08X(%08d) by %s", currentAllocationCount, allocationTypes[allocationType], reportedSize, reportedSize, ownerString(sourceFile, _sourceLine, _sourceFunc));
        ...
        double    a = rand();
        if (a > RAND_MAX / 100.0 * RANDOM_FAILURE)
        {
        ...
        if (_sourceFunc) strncpy(au->sourceFunc, _sourceFunc, sizeof(au->sourceFunc) - 1);
...
void    *m_reallocator(const char *sourceFile, const unsigned int _sourceLine, const char *_sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
    try
    {
        #ifdef TEST_MEMORY_MANAGER
        ...
        if (!reportedAddress)
        {
            return m_allocator(sourceFile, _sourceLine, _sourceFunc, reallocationType, reportedSize);
        }
        ...
        if (alwaysLogAll) log("[~] %05d %8s of size 0x%08X(%08d) by %s", currentAllocationCount, allocationTypes[reallocationType], reportedSize, reportedSize, ownerString(sourceFile, _sourceLine, _sourceFunc));
...
void    m_freeator(const char *sourceFile, const unsigned int _sourceLine, const char *_sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
    try
    {
        #ifdef TEST_MEMORY_MANAGER
        log("[D] ENTER: m_freeator()");
        #endif

        // Log the request

        if (alwaysLogAll) log("[-] ----- %8s of addr 0x%08X           by %s", allocationTypes[deallocationType], reinterpret_cast<unsigned int>(const_cast<void *>(reportedAddress)), ownerString(sourceFile, _sourceLine, _sourceFunc));
