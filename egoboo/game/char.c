...
void chr_log_script_time( const CHR_REF ichr )
{
    ...
    ftmp = vfs_openAppend("/debug/script_timing.txt");
    if ( NULL != ftmp )
    {
...
CHR_REF chr_pack_has_a_stack( const CHR_REF item, const CHR_REF character )
{
    ...
    {
        PACK_BEGIN_LOOP( ChrList.lst[character].inventory, ipitem, iitem )
        {
            cap_t * pstack_cap = chr_get_pcap( iitem );

            found = pstack_cap->isstackable;

            if ( pitem->ammo >= ipitem->ammomax )
            {
                found = bfalse;
            }
            ...
            if ( found && ( pitem->profile_ref != ipitem->profile_ref ) )
            {
