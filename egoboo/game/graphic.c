...
void gfx_init_SDL_graphics()
{
...
#if !defined(__APPLE__)
    {
        ...
        theSurface = IMG_Load_RW(vfs_getReadRWops(fileload), 1);
        if ( NULL == theSurface )
        {
...
float draw_status( const CHR_REF character, float x, float y )
{
    ...
    //Draw the small XP progress bar
    if (cfg.xpbar_allowed) y = draw_character_xp_bar( character, x + 16, y );
...
bool_t dump_screenshot()
{
    ...
    strncpy( szResolvedFilename, szFilename, sizeof( szFilename ) );

    // if we are not using OpenGL, use SDL to dump the screen
    if ( HAS_NO_BITS( sdl_scr.pscreen->flags, SDL_OPENGL ) )
    {
        SDL_SaveBMP_RW( sdl_scr.pscreen, vfs_getWriteRWops(szFilename), 1 );
        return btrue; // why was this returning false, it's supposed to return true if success
    }
    ...
    GL_DEBUG( glPushClientAttrib )( GL_CLIENT_PIXEL_STORE_BIT ) ;
    {
        ...
        if ( -1 != SDL_LockSurface( temp ) )
        {
            ...
            saved = ( -1 != SDL_SaveBMP_RW( temp, vfs_getWriteRWops(szFilename), 1 ) );
        }
