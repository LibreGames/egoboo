...
MD2_Model_t* md2_load( const char * szFilename, MD2_Model_t* mdl )
{
    ...
    vfs_read( &md2_header, sizeof( md2_header ), 1, f );
    ...
    vfs_seek( f , md2_header.offset_st );
    ...
    vfs_seek( f, md2_header.offset_tris );
    vfs_read( model->m_triangles, sizeof( id_md2_triangle_t ), md2_header.num_tris, f );
    ...
    vfs_seek( f, md2_header.offset_skins );
    vfs_read( model->m_skins, sizeof( id_md2_skin_t ), md2_header.num_skins, f );

    // Load the frames of animation
    vfs_seek( f, md2_header.offset_frames );
    for ( i = 0; i < md2_header.num_frames; i++ )
    {
    ...
    if ( md2_header.size_glcmds > 0 )
    {
        ...
        vfs_seek( f, md2_header.offset_glcmds );
        ...
        while ( cmd_size < md2_header.size_glcmds )
        {
            ...
            vfs_read( &commands, sizeof( Sint32 ), 1, f );
            ...
            //read in the data
            vfs_read( cmd->data, sizeof( id_glcmd_packed_t ), cmd->command_count, f );
