...
Mix_Chunk * sound_load_chunk_vfs( const char * szFileName )
{
    ...
    {
        file_exists = btrue;
        tmp_chunk = Mix_LoadWAV_RW( vfs_getReadRWops(full_file_name), 1 );
    }
...
Mix_Music * sound_load_music( const char * szFileName )
{
    ...
    if ( vfs_exists( full_file_name ) )
    {
        ...
        tmp_music = Mix_LoadMUSType_RW( vfs_getReadRWops(full_file_name), MUS_WAV, 1 );
    }
    ...
    if ( NULL == tmp_music )
    {
        ...
        if ( vfs_exists( full_file_name ) )
        {
            file_exists = btrue;
            //tmp_music = Mix_LoadMUS( full_file_name );
            tmp_music = Mix_LoadMUSType_RW( vfs_getReadRWops(full_file_name), MUS_OGG, 1 );
        }
    }
...
void load_all_music_sounds_vfs()
{
    ...
    for ( cnt = 0; cnt < MAXPLAYLISTLENGTH && !vfs_eof( playlist ); cnt++ )
    {
        if ( goto_colon( NULL, playlist, btrue ) )
        {
            ...
            musictracksloaded[cnt] = Mix_LoadMUSType_RW( vfs_getReadRWops( loadpath ), MUS_NONE, 1 );
        }
    }
    musicinmemory = btrue;
    ...
    if ( check_time( SEASON_CHRISTMAS ) )
    {
        ...
        Mix_Music * tmp = Mix_LoadMUSType_RW( vfs_getReadRWops( loadpath ), MUS_NONE, 1 );
        if ( tmp != NULL ) {
            Mix_FreeMusic(musictracksloaded[MENU_SONG]);
            musictracksloaded[MENU_SONG] = tmp;
        }
    }
