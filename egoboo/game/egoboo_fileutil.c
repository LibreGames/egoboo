...
/// @file physfs/egoboo_pfileutil.c
/// @brief Implementation of Egoboo file utilities in pure PhysFS
/// @details
...
char fget_first_letter( vfs_FILE* fileread )
{
    ...
    /// why is this stupid???
    char cTmp = (char) vfs_getc(fileread);
...
bool_t fget_name( vfs_FILE* fileread,  char *szName, size_t max_len )
{
    ...
    fields = fget_string(fileread, szName, max_len);
...
Uint32  ego_texture_load_vfs( oglx_texture_t *texture, const char *filename, Uint32 key )
{
    ...
    else
    {
        ...
        image = SDL_LoadBMP_RW(vfs_getReadRWops(fullname), 1);
...
float fget_damage_resist( vfs_FILE * fileread )
{
    ...
    //else                        resistance = resistance / 100.0f; // resistance is .50 for 50%, not 50
...
static bool_t getString( vfs_FILE * pfile, char * str, size_t str_len, int *numSpaces ) {
    int i;
    int c;
    if (pfile == NULL) return bfalse;
    c = vfs_getc(pfile);
    while (isspace(c)) {
        c = vfs_getc(pfile);
        if (numSpaces != NULL) (*numSpaces)++;
    }
    if (vfs_error(pfile) || vfs_eof(pfile))
        return bfalse;
    for (i = 0; i < str_len; i++) {
        if (c == -1) {
            if (i == 0) return bfalse;
            break;
        }
        if (c == 0) break;
        if (isspace(c))
...
int fget_int( vfs_FILE * pfile ) {
    long pos = vfs_tell(pfile);
    char buf[256];
    char * pbuf;
    int tmp = 0;
    int numSpaces = 0;
    bool_t q = getString(pfile, buf, 256, &numSpaces);
    if (q == bfalse) {
        vfs_seek(pfile, pos);
        return 0;
    }
    tmp = (int)strtol(buf, &pbuf, 10);
    vfs_seek(pfile, pos + pbuf - buf + 1 + numSpaces);
    return tmp;
}

bool_t fget_string( vfs_FILE * pfile, char * str, size_t str_len ) {
    int i;
    int c;
    if (pfile == NULL) return bfalse;
    c = vfs_getc(pfile);
...
float fget_float( vfs_FILE * pfile ) {
    long pos = vfs_tell(pfile);
    char buf[256];
    char * pbuf;
    float tmp = 0;
