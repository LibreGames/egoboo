...
void camera_read_input( camera_t *pcam, input_device_t *pdevice )
{
    ...
    switch ( type )
    {
        ...
        {
            int camleft = control_is_pressed(pdevice, CONTROL_CAMERA_LEFT) == btrue;
            int camright = control_is_pressed(pdevice, CONTROL_CAMERA_RIGHT) == btrue;
            int camin = control_is_pressed(pdevice, CONTROL_CAMERA_IN) == btrue;
            int camout = control_is_pressed(pdevice, CONTROL_CAMERA_OUT) == btrue;
            //Auto camera
            if(autoturn_camera) 
            {
                pcam->turnadd += ( camleft - camright ) * CAM_TURN_KEY;
            }
            ...
            else
            {
                ...
                {
                    pcam->zaddgoto += ( camout - camin ) * CAM_TURN_KEY;
                    if ( pcam->zaddgoto < CAM_ZADD_MIN )  pcam->zaddgoto = CAM_ZADD_MIN;
                    if ( pcam->zaddgoto > CAM_ZADD_MAX )  pcam->zaddgoto = CAM_ZADD_MAX;
                }
            }
