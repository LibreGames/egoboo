...
mpd_t * mpd_load( const char *loadname, mpd_t * pmesh )
{
    ...
    vfs_read( &itmp, 4, 1, fileread );  pinfo->vertcount   = ( int )ENDIAN_INT32( itmp );

    // grab the tiles in x and y
    vfs_read( &itmp, 4, 1, fileread );  pinfo->tiles_x = ( int )ENDIAN_INT32( itmp );
    ...
    vfs_read( &itmp, 4, 1, fileread );  pinfo->tiles_y = ( int )ENDIAN_INT32( itmp );
    ...
    {
        vfs_read( &itmp, 4, 1, fileread ); itmp = ENDIAN_INT32(itmp);
        pmem->tile_list[fan].type = CLIP_TO_08BITS( itmp >> 24 );
        pmem->tile_list[fan].fx   = CLIP_TO_08BITS( itmp >> 16 );
        pmem->tile_list[fan].img  = CLIP_TO_16BITS( itmp >>  0 );
    }

    // Load twist data
    for ( fan = 0; fan < tiles_count; fan++ )
    {
        vfs_read( &itmp, 1, 1, fileread );
