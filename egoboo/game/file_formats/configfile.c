...
ConfigFile_retval ConfigFile_PassOverCommentary( ConfigFilePtr_t pConfigFile )
{
    ...
    lc = vfs_getc( pConfigFile->f );

    while ( C_CARRIAGE_RETURN_CHAR != lc && ASCII_LINEFEED_CHAR != lc && !vfs_eof( pConfigFile->f ) )
    {
        //lc = fgetc( pConfigFile->f );
        lc = vfs_getc( pConfigFile->f );
    }
...
size_t ConfigFile_ReadSectionName( ConfigFilePtr_t pConfigFile, ConfigFileSectionPtr_t pSection )
{
    ...
    while ( '}' != lc && 0 == vfs_eof( pConfigFile->f ) )
    {
    ...
    while ( 0 == lEndScan )
    {
        ...
        switch ( lState )
        {
            case 0:
                ...
                else if ( C_CARRIAGE_RETURN_CHAR == lc || ASCII_LINEFEED_CHAR == lc || vfs_eof( pConfigFile->f ) )
                {
                    // error
                    lEndScan = 1;
                }
    ...
    while ( 0 == lEndScan )
    {
        ...
        switch ( lState )
        {
            ...
            case 1:
                ...
                // check if really _dtor of string
                if ( C_CARRIAGE_RETURN_CHAR == lc || ASCII_LINEFEED_CHAR == lc || vfs_eof( pConfigFile->f ) )
                {
...
ConfigFilePtr_t ConfigFile_open( ConfigFilePtr_t pConfigFile, const char *szFileName, const char * szAttribute, bool_t force )
{
    ...
    if ( local_attribute[0] == 'r' )
        lTempFile = vfs_openRead( szFileName );
    else if ( local_attribute[0] == 'w' )
        lTempFile = vfs_openWrite( szFileName );
    else if ( local_attribute[0] == 'a' )
        lTempFile = vfs_openAppend( szFileName );
    else
        log_error("local_attribute configfile_open??? %s", local_attribute);
    //lTempFile = vfs_open( szFileName, local_attribute );
    if ( NULL == lTempFile )
    {
        return pConfigFile;
    }
...
ConfigFile_retval ConfigFile_read( ConfigFilePtr_t pConfigFile )
{
    ...
    if ( NULL == pConfigFile->f || vfs_eof( pConfigFile->f ) ) return ConfigFile_fail;
