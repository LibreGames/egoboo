...
bool_t input_settings_save_vfs( const char* szFilename )
{
    ...
    for( i = 0; i < MAX_LOCAL_PLAYERS; i++ )
    {
        ...
        export_control( filewrite, "Open Left Inventory  ", pdevice->device_type, pdevice->control + CONTROL_INVENTORY );
        //export_control( filewrite, "Open Right Inventory ", pdevice->device_type, pdevice->control + CONTROL_RIGHT_INVENTORY );
